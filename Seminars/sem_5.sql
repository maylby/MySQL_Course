...
(
    SELECT 1 AS n, 1 AS res -- стартовая позиция 1 (столбец "n") и 1 (столбец "res")
    UNION ALL -- объединенить всё (свести данные в одну таблицу)
    SELECT n + 1 AS n, -- прибавляем 1 к предыдущему значению "n"
					   -- и присваиваем новое значение переменной "n", т.е.
                       -- переходим к следующему числу
		POW(n + 1, 2) AS res -- текущее значение "n + 1" возводим во вторую степень и
							 -- передаём полученный результат параметру "res"
    FROM cte_rec -- из табличного выражения "cte_rec"
    WHERE n + 1 <= 10 -- пока текущее число столбца "n" не станет равным 10
)
SELECT * FROM cte_rec;
